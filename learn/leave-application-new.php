...
<body>
    ...
    <?php
        ...
        $sql="SELECT emp_name,emp_id FROM `lms_miniproject`.`employee` WHERE (username='$username')";
        $result=$conn->query($sql);
        $row=$result->fetch_row();
        $_SESSION['empname']=$row[0];
        $_SESSION['empid']=$row[1];
    ?>

    <form action="leave-application.php" id="leaveApply" method="POST">
    <div id="Details">
        <label>Employee ID</label>
        <input type="text" id="Employee_id" readonly value="<?php
            echo $_SESSION['empid'];
        ?>">

        <label>Name</label>
        <input type="text" id="Name" readonly  value="<?php
